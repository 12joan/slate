...
const replaceChild = <T>(xs: T[], index: number, newValue: T) => xs.map((x, i) => i === index ? newValue : x)

const modifyDescendant = <N extends Descendant>(editor: Editor, path: Path, f: (node: N) => N) => {
  if (path.length === 0) {
  ...
  const index = slicedPath.pop()!
  editor.children = replaceChild(editor.children, index, modifiedNode)
}

const modifyChildren = (editor: Editor, path: Path, f: (children: Descendant[]) => Descendant[]) => {
  if (path.length === 0) {
...
const applyToDraft = (editor: Editor, op: Operation) => {
